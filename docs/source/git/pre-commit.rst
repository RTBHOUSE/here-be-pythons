...
+ You can find an example of pre-commit Git hook in `Big-Bang-py <https://github.com/RTBHOUSE/big-bang-py/blob/master/%7B%7Bcookiecutter.project_dir%7D%7D/hooks/pre-commit>`_.

    + ``install_precommit`` Invoke task in `Big-Bang-py <https://github.com/RTBHOUSE/big-bang-py/blob/master/%7B%7Bcookiecutter.project_dir%7D%7D/tasks.py#L27-L35>`_ takes your pre-commit Git Hook from ``/hooks/pre-commit`` and automatically sets it up for you.
